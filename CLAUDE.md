...
Many of these files are generated by cyto when mapping and demultiplexing Flex-Probes (one-file-per-probe).
This is a deterministically sized format with a HEADER, and then many 24-bit records, each containing 2bit encoded BARCODE and UMI fields (u64 each) and a u64 representing the library element mapped by the record.
...
- `cyto-map` - Maps reads to features (barcode correction, geometry handling)
