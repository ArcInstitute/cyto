...
fn main() -> Result<()> {
    ...
    match args.command {
        Commands::Map(map) => match map {
            MapCommand::Gex(args) => {
                if args.map.probe_path().is_some() {
                    cyto_map::run_gex(&args)
                } else {
                    cyto_map::run_gex_unprobed(&args)
                }
            }
            MapCommand::Crispr(args) => {
                if args.map.probe_path().is_some() {
                    cyto_map::run_crispr(&args)
                } else {
                    cyto_map::run_crispr_unprobed(&args)
                }
            }
        },
