...
- `src/detect.rs` — Geometry detection module. Samples N reads **per input file (lane)** independently, scans all positions for known reference sequences, then reconciles lanes in `finalize_detection`: (1) **pools** all per-lane `PositionAccumulator`s and infers the **reported geometry from the pool** (sharpens estimates for shallow lanes); (2) infers each lane with the accept threshold disabled so weak lanes still yield a result; (3) **accepts on the per-lane maximum** — detection succeeds iff **at least one lane** matches every component at or above `min_proportion` (a single low-signal lane cannot cause failure); (4) checks **cross-lane consistency** by per-component position tolerance against the pool (feature components tolerate the max remap window across pool and lanes — computed before this check — barcode/probe a fixed 2), where only components a lane is *strong* on vote; (5) reports the **maximum remap window** across the pool and all lanes (advisory — the mapper uses `--remap-window`, not this value) and re-walks the pooled evidence's windowed counts at that window. A lane is fatal only on zero reads, an unresolvable overlap, or a missing required component (`infer_geometry` bails naming the component instead of panicking). Types: `DetectionConfig` (`#[derive(Clone, Copy)]`; `num_reads`, `min_proportion`, `remap_min_proportion`, `num_threads`), `ComponentEvidence` (single-position `match_count`/`match_proportion` plus windowed counterparts `windowed_match_count`/`windowed_match_proportion` summed over `[pos ± remap_window]` on the same mate — approximates what `cyto map --remap-window N` would score; for short references like the 8bp `[probe]` a read can hit multiple in-window positions, so windowed values are an upper bound and may exceed 1.0), `PerFileResult` (per-lane detection summary), `DetectionResult` (includes `per_file_results: Vec<PerFileResult>`, one entry per input lane including weak ones). Public API: `detect_gex_geometry()`, `detect_crispr_geometry()` (both consume unpositioned mappers, sample all input files, pool+accept+validate via `finalize_detection`), `log_detection_result()` (logs the aggregated result to stderr via `info!`; emits a `Recommended --remap-window: N` line on its own for copy-paste into `cyto map`, and per-component lines carrying `windowed_count=`/`windowed_proportion=` inline after `count=`/`proportion=`; the bare geometry string itself is printed to stdout by `run_detect_*` in `run.rs`, not by this module). Internal: `PositionAccumulator` (with `windowed_count()` helper summing hits over `[best_pos.saturating_sub(window), best_pos + window]` inclusive on a given `(component, mate)`), `GexDetectionProcessor`/`CrisprDetectionProcessor` (implement both `binseq::ParallelProcessor` and `paraseq::PairedParallelProcessor`), `infer_geometry()` (bails on a missing required component; populates windowed evidence at the estimated window), `find_best_positions()` (total tie-break sort so single-lane output and lane-vs-pool comparison are deterministic across independently-seeded HashMaps), `estimate_remap_window()` (proportion-based threshold, contiguous-range walk, total tie-break argmax; excludes only `[barcode]` — `[probe]` is included to capture V2 `[:18]` spacer drift), `resolve_overlaps()`, `finalize_detection()` + `component_tolerance()` (pooled estimate, per-lane-max accept, tolerance consistency, advisory max remap window, windowed re-walk), `log_per_file_result()`, `log_top_alternatives()`.
