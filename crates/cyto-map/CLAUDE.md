...
- `src/detect.rs` — Geometry detection. Samples N reads **per input file (lane)** independently, scans all positions for known reference sequences, then reconciles lanes in `finalize_detection`: pools all lanes and infers the reported geometry from the pool (sharpens shallow lanes); infers each lane with the accept threshold disabled; **accepts if at least one lane** matches every component at or above `min_proportion` (one low-signal lane cannot fail detection); checks cross-lane consistency by per-component position tolerance against the pool (only components a lane is *strong* on vote); reports the maximum remap window across pool and lanes (advisory — the mapper uses `--remap-window`, not this value). A lane is fatal only on zero reads, an unresolvable overlap, or a missing required component (`infer_geometry` bails naming it instead of panicking). Types: `DetectionConfig` (`num_reads`, `min_proportion`, `remap_min_proportion`, `num_threads`), `ComponentEvidence` (single-position `match_count`/`match_proportion` plus windowed `windowed_match_count`/`windowed_match_proportion` summed over `[pos ± remap_window]` on the same mate — approximates `cyto map --remap-window N`; short references like the 8bp `[probe]` can hit multiple in-window positions, so windowed values may exceed 1.0), `PerFileResult` (per-lane summary), `DetectionResult` (holds `per_file_results`, one per lane). Public API: `detect_gex_geometry()`, `detect_crispr_geometry()` (consume unpositioned mappers, sample all files, reconcile via `finalize_detection`), `log_detection_result()` (logs a `Recommended --remap-window: N` line and per-component evidence to stderr; the geometry string itself is printed to stdout by `run_detect_*` in `run.rs`). Internal: `PositionAccumulator` (with `windowed_count()`), `GexDetectionProcessor`/`CrisprDetectionProcessor`, `infer_geometry()`, `build_evidence()`, `find_best_positions()`, `estimate_remap_window()` (excludes `[barcode]` only; `[probe]` included for V2 spacer drift), `resolve_overlaps()`, `finalize_detection()` + `component_tolerance()` + `collect_lane_conflicts()`, `log_per_file_result()`, `log_top_alternatives()`.
...
- `src/mapper/mod.rs` — `Mapper` trait (`query(&self, seq) -> Option<FeatureMatch>`, `mate() -> ReadMate`), `FeatureMatch` (`feature_idx`, `end_pos`), `Library` trait (statistics), typestate markers (`Unpositioned`, `Ready`)
...
- `src/mapper/crispr.rs` — `CrisprMapper<S>`: two-stage matching — `MultiLenSeqHash` for variable-length anchors, then `SeqHash` for fixed-length protospacers (protospacer offset computed dynamically from the anchor match). Guide identity is the `(anchor, protospacer)` pair via **disambiguate-on-collision**: protospacers are deduplicated before hashing (so `SeqHash` never sees a duplicate parent), a unique protospacer maps to its guide regardless of anchor (backward-compatible), and a protospacer shared across anchors is disambiguated by the matched anchor. `proto_guides` maps each protospacer parent index to its `(anchor_global_idx, feature_idx)` guides; a matched anchor carrying none of them yields no match. `feature_idx` is the 0-based TSV row index (contiguous `0..N`, aligned in order with `FeatureWriter::record_stream()` / `metadata/features.tsv`). Two rows identical in **both** anchor and protospacer are a load-time error naming both guides.
...
Unit tests: `src/geometry.rs` (parsing/resolution) and `src/detect.rs` (position accumulation, `infer_geometry`, remap-window estimation, `finalize_detection` reconciliation). End-to-end detection over `data/` is in `tests/detect_integration.rs`. Mapping integration tests use `justfile` targets:
