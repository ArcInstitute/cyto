...
fn write_counts_mtx<P: AsRef<Path>>(
    ...
    eprintln!("Creating output directory: {}", outdir.as_ref().display());
    ...
    eprintln!("Creating mtx file: {}", mtx_path.display());
    let mtx_handle = match_output(Some(mtx_path))?;
    eprintln!("Creating barcodes file: {}", barcodes_path.display());
    let barcodes_handle = match_output(Some(barcodes_path))?;
    eprintln!("Creating features file: {}", features_path.display());
